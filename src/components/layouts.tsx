...
function PatientTopBar() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-brand-100/70 bg-canvas/85 backdrop-blur">
...
export function CaregiverShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-brand-100 bg-white md:flex">
        ...
        <nav className="flex flex-1 flex-col gap-1.5 px-4" aria-label="Caregiver">
          {CAREGIVER_NAV.map(({ to, label, Icon, end }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-2xl px-4 py-3 text-lg font-bold transition ${
      ...
      <div className="flex min-h-screen flex-1 flex-col">
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-brand-100 bg-canvas/90 px-4 py-3 backdrop-blur md:px-8">
...
function OfflineBanner() {
  ...
  if (isOffline) {
    return (
      <div className="mx-4 mt-4 flex items-center gap-2 rounded-2xl border-2 border-warm-300 bg-warm-100 px-4 py-3 text-base font-bold text-warm-700">
        🟠 {t('offline.banner')}{' '}
